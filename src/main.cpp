...
namespace {
    ...
    enum class HubTab { Player, Assist, Visual, Utility };
    ...
    void setDebugDrawEnabled(PlayLayer* playLayer, bool enabled) {
        if (!playLayer) return;
        if (playLayer->shouldDebugDraw() != enabled) playLayer->toggleDebugDraw();
    ...
    void applyPlayLayerOptions(PlayLayer* playLayer) {
        ...
        if (g_practiceMode) playLayer->togglePracticeMode(true);
    ...
    void releaseAutoButtons(PlayerObject* player = nullptr) {
        if (!player) {
            if (auto playLayer = PlayLayer::get()) player = playLayer->m_player1;
        }
        if (!player) {
    ...
    void setJumpHeld(PlayerObject* player, bool held) {
        ...
        if (held) player->pushButton(kJumpButton);
        else player->releaseButton(kJumpButton);
        g_autoHoldingJump = held;
    }

    void startJumpTap(PlayerObject* player, int frames = 2, int cooldown = 7) {
        if (!player || g_autoTapCooldown > 0) return;
    ...
    bool updateJumpTap(PlayerObject* player) {
        if (g_autoTapCooldown > 0) --g_autoTapCooldown;
        if (g_autoTapFrames <= 0) return false;
        --g_autoTapFrames;
        setJumpHeld(player, true);
        if (g_autoTapFrames == 0) setJumpHeld(player, false);
        return true;
    }

    void setRightHeld(PlayerObject* player, bool held) {
        if (!player || g_autoHoldingRight == held) return;
        if (held) player->pushButton(kRightButton);
        else player->releaseButton(kRightButton);
    ...
    PlayerMode currentPlayerMode(PlayerObject* player) {
        ...
        if (player->isInNormalMode() || player->isInBasicMode()) return PlayerMode::Cube;
    ...
    bool looksLikeGameplayCollision(GameObject* object) {
        if (!object || !object->isVisible() || object->m_isDisabled || object->m_isGroupDisabled || object->m_isTrigger) return false;
        if (object->m_isDecoration || object->m_isDecoration2 || object->m_isPassable || object->m_isNoTouch || object->m_isInvisible) return false;
    ...
    std::vector<GameObject*> collectNearbyCollisionObjects(PlayLayer* layer, PlayerObject* player, float lookAhead, float verticalRange = 320.f) {
        ...
        if (!layer || !player || !layer->m_objectLayer) return objects;
        auto playerPos = player->getPosition();
        for (auto node : layer->m_objectLayer->getChildrenExt()) {
            auto object = typeinfo_cast<GameObject*>(node);
            if (!looksLikeGameplayCollision(object)) continue;
            auto objectPos = object->getPosition();
            auto dx = objectPos.x - playerPos.x;
            if (dx < -55.f || dx > lookAhead) continue;
            if (std::abs(objectPos.y - playerPos.y) > verticalRange) continue;
    ...
    bool shouldGroundJump(PlayLayer* layer, PlayerObject* player, PlayerMode mode) {
        ...
        if (!scan.obstacleAhead && !scan.wallAhead) return false;
        ...
        if (mode == PlayerMode::Ball || mode == PlayerMode::Spider) return grounded && nearEnough;
        if (mode == PlayerMode::Robot) return nearEnough && (grounded || (player->m_isUpsideDown ? yVelocity > 0.5 : yVelocity < -0.5));
    ...
    bool shouldFlightHold(PlayLayer* layer, PlayerObject* player, PlayerMode mode) {
        ...
        if (playerPos.y < softBottom || scan.obstacleBelow || scan.wallAhead) return !player->m_isUpsideDown;
        if (playerPos.y > softTop || scan.obstacleAbove) return player->m_isUpsideDown;
    ...
    void runAutoPlay(PlayLayer* layer) {
        ...
        if (g_platformerAssist) setRightHeld(player, true);
        else setRightHeld(player, false);

        if (updateJumpTap(player)) return;
        ...
        if (isGroundMode(mode) && g_autoCube) {
            ...
            if (mode == PlayerMode::Ball || mode == PlayerMode::Spider) {
                if (wantsJump) startJumpTap(player, 1, mode == PlayerMode::Spider ? 9 : 7);
                else setJumpHeld(player, false);
                return;
            }
        ...
        if (!isGroundMode(mode) && g_autoWave) {
            if (mode == PlayerMode::Ufo || mode == PlayerMode::Swing) {
                if (shouldTapFlight(layer, player, mode)) startJumpTap(player, 2, mode == PlayerMode::Ufo ? 10 : 8);
                else setJumpHeld(player, false);
                return;
            }
...
public:
    ...
    bool ccTouchBegan(CCTouch* touch, CCEvent*) override {
        ...
        if (m_open && nodeContainsWorldPoint(m_panel, worldPoint)) return true;
        return false;
    }
    ...
    void ccTouchMoved(CCTouch* touch, CCEvent*) override {
        ...
        auto winSize = CCDirector::sharedDirector()->getWinSize();
        auto worldPoint = touch->getLocation();
        if (worldPoint.getDistance(m_touchStart) > 6.f) m_movedTouch = true;
    ...
    void ccTouchEnded(CCTouch*, CCEvent*) override {
        if (m_dragging && !m_movedTouch) this->togglePanel();
        m_dragging = false;
        m_movedTouch = false;
    }
...
private:
    ...
    void switchTab(HubTab tab) {
        m_tab = tab;
        if (m_playerPage) m_playerPage->setVisible(tab == HubTab::Player);
        if (m_assistPage) m_assistPage->setVisible(tab == HubTab::Assist);
        if (m_visualPage) m_visualPage->setVisible(tab == HubTab::Visual);
        if (m_utilityPage) m_utilityPage->setVisible(tab == HubTab::Utility);
        
        if (m_tabTitleLabel) {
            switch (tab) {
                case HubTab::Player: m_tabTitleLabel->setString("Player Controls"); break;
                case HubTab::Assist: m_tabTitleLabel->setString("Assist Suite"); break;
                case HubTab::Visual: m_tabTitleLabel->setString("Visual Tools"); break;
                case HubTab::Utility: m_tabTitleLabel->setString("Utility Deck"); break;
            }
        }
    }
    ...
    void tickStatus(float) {
        ...
        if (!m_statusLabel || !playLayer) return;

        auto percent = playLayer->getCurrentPercent();
        char buffer[128];
        std::snprintf(
            buffer, sizeof(buffer), "%.2f%% | %s %s %.2fx %s",
            percent,
    ...
    void onTogglePractice(CCObject*) {
        ...
        if (auto playLayer = PlayLayer::get()) playLayer->togglePracticeMode(g_practiceMode);
        this->syncStateLabels();
    ...
    void onToggleAutoPlay(CCObject*) {
        ...
        if (!g_autoPlay) releaseAutoButtons();
        this->syncStateLabels();
    ...
    void onTogglePlatformer(CCObject*) {
        ...
        if (!g_platformerAssist) releaseAutoButtons();
        this->syncStateLabels();
    ...
    void onToggleHidePlayer(CCObject*) {
        ...
        if (auto playLayer = PlayLayer::get()) playLayer->togglePlayerVisibility(!g_hidePlayer);
        this->syncStateLabels();
    ...
    void onToggleHideAttempts(CCObject*) {
        ...
        if (auto playLayer = PlayLayer::get()) playLayer->toggleHideAttempts(g_hideAttempts);
        this->syncStateLabels();
    ...
    void onToggleHideGround(CCObject*) {
        ...
        if (auto playLayer = PlayLayer::get()) playLayer->toggleGroundVisibility(!g_hideGround);
        this->syncStateLabels();
    ...
    void onToggleHideMG(CCObject*) {
        g_hideMG = !g_hideMG;
        if (auto playLayer = PlayLayer::get()) playLayer->toggleMGVisibility(!g_hideMG);
        this->syncStateLabels();
    ...
    void onToggleBGEffects(CCObject*) {
        ...
        if (auto playLayer = PlayLayer::get()) playLayer->toggleBGEffectVisibility(g_bgEffects);
        this->syncStateLabels();
    ...
    void onToggleHitboxes(CCObject*) {
        ...
        if (auto playLayer = PlayLayer::get()) setDebugDrawEnabled(playLayer, g_showHitboxes);
        this->syncStateLabels();
    ...
    void onSpeedUp(CCObject*) {
        ...
        if (auto playLayer = PlayLayer::get()) playLayer->updateTimeMod(currentSpeed(), true, true);
        this->syncStateLabels();
